...
Player.onConnect = function(socket){ //this should only use socket.id maybe
    ...
    socket.on('keyPress',function(data){
        if(data.inputId === 'left')
			...
			player.mouseX = data.mouseX - player.x;
			player.mouseY = data.mouseY - player.y;
			player.aimAngle = Math.atan2(player.mouseY,player.mouseX) / Math.PI * 180;
		}
    });
}
...
//////////////////////////////////////////////////////////////
//Bullet - uses Entity
Bullet = function (id,x,y,spdX,spdY,width,height,combatType,parent){
	...
	//self.spdY = spdY; not needed as spdY is in Entity
	...
	self.update = function () {
		...
		//bullet collision
		for(var key in Player.list){
			var p = Player.list[key];
			if(self.testCollision(p) && self.parent !==p.id){  ///////////TESTCOLLISION
				self.toRemove = true;
				Player.list[key].hp -= 1;
